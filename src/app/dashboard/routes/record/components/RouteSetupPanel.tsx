          ...
          {!existingOnly && (
            <>
              <button type="button" onClick={onCreateRoute}
                className="flex w-full items-center justify-between gap-2 rounded-xl border border-gdh-brand/30 bg-gdh-brand/10 px-4 py-3 text-left transition hover:bg-gdh-brand/15"
              >
                <span className="font-medium text-white">Nueva ruta libre</span>
                <ChevronRight className="shrink-0 text-gdh-brand-highlight" size={20} />
              </button>
              <div className="rounded-xl border border-white/10 bg-[#12161c] p-3">
                <p className="mb-2 text-xs text-slate-400">
                  ...
                  <span className="text-gdh-brand-highlight">Centrar mapa en mi GPS</span>).
                </p>
              ...
              <div className="rounded-xl border border-white/10 bg-[#12161c] p-3">
                ...
                <button type="button" disabled={tileDownloadBusy || (!mapBootstrapPos && !selectedRouteForPreview)}
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-gdh-brand px-3 py-2 text-sm font-semibold text-white hover:bg-gdh-brand-highlight disabled:opacity-45"
                >
                ...
                {tileDownloadProgress && (
                  <p className="mt-2 text-xs text-slate-300">Progreso: {tileDownloadProgress.done}/{tileDownloadProgress.total}</p>
                )}
            ...
            <div className="space-y-3 border-t border-white/10 pt-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <button type="button" onClick={onRefreshDistance} disabled={checkingPosition}
                  className="text-xs font-medium text-gdh-brand-highlight underline-offset-2 hover:underline disabled:opacity-50"
                >
              ...
              <p className="text-xs text-slate-400">
                ...
                {distanceToStartM != null && (
                  <> Ahora: <strong className="text-gdh-brand-highlight">{Math.round(distanceToStartM)} m</strong></>
                )}
