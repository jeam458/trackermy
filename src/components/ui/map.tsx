...
type MapProps = {
  ...
  /**
   * Estilo fijo (URL o spec MapLibre). Si se define, tiene prioridad sobre `theme` + `styles`
   * para la URL del estilo (útil para varios basemaps vectoriales en la misma vista).
   */
  forceStyle?: MapStyleOption;
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const activeStyle = useMemo<MapStyleOption>(() => {
    if (forceStyle != null) return forceStyle;
    return resolvedTheme === "dark" ? mapStyles.dark : mapStyles.light;
  }, [forceStyle, resolvedTheme, mapStyles.dark, mapStyles.light]);
  ...
  useEffect(() => {
    ...
    const initialStyle = activeStyle;
  ...
  useEffect(() => {
    ...
    clearStyleTimeout();
    const prev = currentStyleRef.current;
    currentStyleRef.current = newStyle;
    setIsStyleLoaded(false);

    const asStr = (s: MapStyleOption | null) =>
      typeof s === "string" ? s : "";
    const canDiff =
      asStr(prev).includes("basemaps.cartocdn.com") &&
      asStr(newStyle).includes("basemaps.cartocdn.com");

    mapInstance.setStyle(newStyle, { diff: canDiff });
...
type MapControlsProps = {
  ...
  /** Contenido extra al final de la columna (p. ej. selector de estilo / capas) */
